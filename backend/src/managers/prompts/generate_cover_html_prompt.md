...
- 这是风格预览 HTML，不是最终 PPT 产物；必须是只读预览，禁止加入编辑模式、编辑按钮、`contenteditable`、localStorage 自动保存、导出按钮或 `InlineEditor` 类
...
- 禁止用 `<img>` 做全屏背景
- 禁止输出 ```html ... ``` 代码块包裹整个输出
- 禁止加入任何编辑能力：不要生成编辑热区、编辑按钮、`contenteditable`、自动保存、导出 HTML、`InlineEditor` 或类似脚本
# 根据风格模版生成示例首页 HTML
...
如果风格模版包含标准化 `page_layouts`，必须优先读取 `page_layouts.cover` 中的 `cover.*` 布局变体、中文布局名、结构和容量约束；只有缺少该结构时，才回退读取自然语言的 Cover / 封面页描述。
