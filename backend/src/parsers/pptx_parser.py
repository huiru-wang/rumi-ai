...
用法:
    ...
    md_text, image_map = parse_pptx_to_markdown(
        "/path/to/file.pptx",
        "/data/rumi-ai/workspace_work/{workspace_id}/style_extract/{task_id}",
    )

    # 带图片资源公开 URL 替换:
    md_text, image_map = parse_pptx_to_markdown(
        "/path/to/file.pptx", "/data/rumi-ai/workspace_work/{workspace_id}/style_extract/{task_id}",
...
def extract(
    ...
    if unpack_dir is None:
        raise ValueError("unpack_dir is required")
    pptx_dir = unpack_dir
    ...
    try:
        ...
        raise ValueError(f"not a valid PPTX/ZIP archive: {pptx_path}") from exc
    ...
    if slides_dir.exists():
        ...
        for slide_file in slide_files:
            ...
            slide_data["slide_no"] = len(slides) + 1
    ...
    return {
        "deck_meta": {"page_size": page_size, "slide_count": len(slides)},
        "metadata": {"pageSize": page_size},
        "theme": theme,
        "slides": slides,
        "media": [
            {"filename": filename, "path": path}
            for filename, path in media_files.items()
        ],
...
def parse_pptx_to_markdown(
    ...
    data = extract(
        ...
        unpack_dir=Path(unpack_dir) if unpack_dir else Path(output_dir) / "pptx_unpack",
    )
    md_text = render(data)
...
def parse_pptx_to_structured(
    pptx_path: str,
    output_dir: str,
    unpack_dir: str | None = None,
) -> dict:
    """Parse PPTX into stable structured data and extract media resources."""
    return extract(
        Path(pptx_path),
        resource_dir=Path(output_dir) / "resource",
        unpack_dir=Path(unpack_dir) if unpack_dir else Path(output_dir) / "pptx_unpack",
...
def write_parse_outputs(data: dict, output_dir: str, resource_base_url: str = "") -> tuple[str, Path, Path]:
    """Persist the structured and readable parser outputs."""
    output_path = Path(output_dir)
    markdown_text = render(data)
    if resource_base_url:
        markdown_text = _replace_image_paths(markdown_text, resource_base_url)
    parsed_json = output_path / "parsed.json"
    parsed_md = output_path / "parsed.md"
    parsed_json.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
    parsed_md.write_text(markdown_text, encoding="utf-8")
    return markdown_text, parsed_json, parsed_md
